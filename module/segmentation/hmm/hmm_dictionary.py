...
class HmmDictionary(Dictionary):
    ...
    def decoding(text, label):
        ...
        :param label: <list> 文本列表对应的标签。要求 text 和 label 的长度必须一致
    ...
    def __init_word_token(self, data):
        ...
        :param data: <list> 数据列表。格式：[['小','明','是','中','国','人']...]
        :return:
        """
        word_count = Counter()
        for d in tqdm(data):
    ...
    def __init_launch_matrix(self, data, label):
        """
        初始化发射矩阵
        :param data: <list> 数据列表 例如：[['小','明','是','中','国','人']...]
    ...
    def fit(self, texts, split_lab):
        """
        拟合
        :param texts: <list> 文本数据集。例如 ["小明 是 中国人",...]
        :param split_lab: <str> 分词标签。例如 ["小明 是 中国人",...] 分词标记为 " "
        :return:
        """
        # 初始化数据标签
        data, label = HmmDictionary.encoding(texts, split_lab)
        # 初始化 word_token
    ...
    def get_launch_prob(self, word, state):
        ...
        获取发射概率
        :param word: <str> 中文字
        :param state: <str> 中文字对应的状态值。"BMES" 中的任一个
    ...
    def get_state_transition_prob(self, state, is_last_state=False):
        ...
        :param state: <str> 当前状态值
        :param is_last_state: <bool> 是否是最后一个状态值。如果为 True 则返回当前状态到 <End> 概率
...
def test():
    texts = ["小明 是 中国人", "小红 爱 中国"]
    hmm_dict = HmmDictionary()
    hmm_dict.fit(texts, split_lab=" ")

    print("launch prob： '小' word state is 'B' prob: ", hmm_dict.get_launch_prob('小', 'B'))
    print("trans prob: '<start> next state prob: {}'".format(hmm_dict.get_state_transition_prob(Dictionary.START_TAG)))
    print("trans prob: 'B' next state prob: {}".format(hmm_dict.get_state_transition_prob(HmmDictionary.STATE_B)))

    # ['小','明','是','中','国','人'] 和 ['B','E','S','B','M','E']
    text_list = ['小', '明', '是', '中', '国', '人']
    label = ['B', 'E', 'S', 'B', 'M', 'E']
    result = HmmDictionary.decoding(text_list, label)
