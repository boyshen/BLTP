...
class AppException(Exception):
    ...
    @staticmethod
    def print_format(name, level, msg, obj):
        """
        异常信息的格式化输出
        :param name: <str> 异常的名称
        :param level: <str> 异常的级别
        :param msg: <str> 异常提示信息
        :param obj: <str> 其他需要输出的信息
        :return: <str> 格式化字符串
        """
        name = AppException.color[level](name)
        level = AppException.color[level](level)
        return "{}: [level:{}] {}: {}".format(name, level, msg, obj)
...
class FileNotFoundException(AppException):
    ...
    def __init__(self, filename, level=AppException.error):
        """
        初始化操作
        :param filename: <str> 文件名，报错的文件名
        :param level: <int> 级别，报错类型的级别
        """
...
def handler(e, is_custom_exception):
    ...
    :param is_custom_exception: <bool> 是否为自定义的异常类型。
    :return:
    """
    print(e)
